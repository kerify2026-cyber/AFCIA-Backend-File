...
async function interpretCode(code) {
  // STEP 1: fault_code_cache (already-generated full reports)
  const cached = await pool.query('SELECT * FROM fault_code_cache WHERE code = $1', [code]);
  if (cached.rows.length) {
    return cacheRowToResult(cached.rows[0]);
  }

  // STEP 2: fault_codes reference table (known code -> title/description/severity)
  const known = await pool.query('SELECT * FROM fault_codes WHERE code = $1', [code]);

  let data;
  if (known.rows.length) {
    // STEP 3: found in DB — ask GPT to expand into the full report shape
    const row = known.rows[0];
    const text = await callAI({
      system: DB_ENRICH_SYSTEM_PROMPT,
      messages: [{
        role: 'user',
        content: `Fault code: ${code}\nKnown title: ${row.title || 'unknown'}\nKnown description: ${row.description || 'unknown'}\nKnown severity hint: ${row.severity || 'unknown'}`
      }]
    });
    data = extractJson(text);
  } else {
    // STEP 6: not found anywhere — GPT researches the code from scratch
    const text = await callAI({
  ...
  await saveToCache(code, data, known.rows.length ? 'db+ai' : 'ai');
