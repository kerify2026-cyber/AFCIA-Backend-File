...
async function findFaultCode(code) {
  const { rows } = await pool.query('SELECT * FROM "Fault_codes" WHERE code = $1', [code]);
  if (!rows.length) return null;
  return rows[0];
}
...
async function interpretCode(code) {
  ...
  if (cached) {
    // STEP 2: found -> return cached result immediately
    return cached;
  }

  // STEP 3: check "Fault_codes" reference table
  const known = await findFaultCode(code);

  let data;
  if (known) {
    // STEP 4: found in "Fault_codes" -> send title/description/severity to GPT
    // to generate the full report shape
    const text = await callAI({
      system: DB_ENRICH_SYSTEM_PROMPT,
      messages: [{
        role: 'user',
        content: `Fault code: ${code}\nKnown title: ${known.title || 'unknown'}\nKnown description: ${known.description || 'unknown'}\nKnown severity hint: ${known.severity || 'unknown'}\nManufacturer: ${known.manufacturer || 'unknown'}\nCategory: ${known.category || 'unknown'}`
      }]
    });
    data = extractJson(text);
  } else {
    // STEP 7: not found in "Fault_codes" either -> ask GPT to research/interpret
  ...
  // STEP 5: save generated result to fault_code_cache
  await saveToCache(code, data, known ? 'db+ai' : 'ai');
